...
def prepare_for_sensor_qc(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    raw_defaults = {
        ...
        "out_temp": np.nan,
        "out_hum": np.nan,
        "out_winddirec": np.nan,
        "out_windsp": 0.0,
        "out_light": 0.0,
        "out_rain": 0.0,
        "cont_skyl_vol": 0.0,
        "cont_fan_run": 0.0,
        "cont_heater_run": np.nan,
    }
    ...
    if "in_hum" in df.columns:
        df["in_hum"] = df["in_hum"].clip(0.0, 100.0)

    if "cont_skyl_vol" in df.columns:
        w = df["cont_skyl_vol"]
        if w.max() <= 1.5:
            w = w * 100.0
        df["cont_skyl_vol"] = w.clip(0.0, 100.0)

    if "out_windsp" in df.columns:
        df["out_windsp"] = df["out_windsp"].clip(lower=0.0)

    for col in ["out_temp", "out_light", "out_windsp"]:
        if col in df.columns:
