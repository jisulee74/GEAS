...
class ConstantTimesNetQualityModel(TimesNetQualityModel):
    def reconstruct(
        self,
        df: pd.DataFrame,
...
class TimesNetQualityModelTest(unittest.TestCase):
    def test_fit_rejects_action_columns_before_torch_is_required(self):
        ...
        with self.assertRaisesRegex(ValueError, "Action columns are not allowed"):
            TimesNetQualityModel().fit(df, ["in_temp", "cont_skyl_vol"])
    ...
    def test_fit_reports_torch_optional_dependency_when_unavailable(self):
        ...
        with patch(
            ...
            TimesNetQualityModel().fit(df, ["in_temp"])
    ...
    def test_predict_outlier_returns_scores_flags_and_confidence(self):
        ...
        model = ConstantTimesNetQualityModel()
    ...
    def test_pipeline_factory_supports_timesnet_without_defaulting_to_it(self):
        model = _make_quality_model("timesnet")

        self.assertIsInstance(model, TimesNetQualityModel)
        self.assertNotIn("timesnet", DEFAULT_CANDIDATE_MODELS)
    ...
    def test_config_from_hyperparameter_candidate_uses_common_and_model_specific(self):
        ...
        candidate = HyperparameterCandidate(
            ...
            model_specific={
                "period_embedding_dim": 4,
                "temporal_blocks": 1,
                "top_k_periods": 1,
            },
        )

        config = TimesNetConfig.from_candidate(candidate)

        self.assertEqual(config.lookback, 3)
        self.assertEqual(config.mask_fraction, 0.5)
        self.assertEqual(config.period_embedding_dim, 4)
        self.assertEqual(config.temporal_blocks, 1)
        self.assertEqual(config.top_k_periods, 1)
