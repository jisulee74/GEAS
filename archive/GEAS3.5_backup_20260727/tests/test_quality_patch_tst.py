...
class ConstantPatchTSTQualityModel(PatchTSTQualityModel):
    def reconstruct(
        self,
        df: pd.DataFrame,
...
class PatchTSTQualityModelTest(unittest.TestCase):
    def test_fit_rejects_action_columns_before_torch_is_required(self):
        ...
        with self.assertRaisesRegex(ValueError, "Action columns are not allowed"):
            PatchTSTQualityModel().fit(df, ["in_temp", "cont_skyl_vol"])
    ...
    def test_fit_reports_torch_optional_dependency_when_unavailable(self):
        ...
        with patch(
            ...
            PatchTSTQualityModel().fit(df, ["in_temp"])
    ...
    def test_predict_outlier_returns_scores_flags_and_confidence(self):
        ...
        model = ConstantPatchTSTQualityModel()
    ...
    def test_pipeline_factory_supports_patch_tst_without_defaulting_to_it(self):
        model = _make_quality_model("patch_tst")

        self.assertIsInstance(model, PatchTSTQualityModel)
        self.assertNotIn("patch_tst", DEFAULT_CANDIDATE_MODELS)
    ...
    def test_config_from_hyperparameter_candidate_uses_common_and_model_specific(self):
        ...
        candidate = HyperparameterCandidate(
            name="small",
            common={"lookback": 4, "mask_fraction": 0.5},
            model_specific={
                "patch_length": 2,
                "patch_stride": 1,
                "transformer_depth": 1,
        ...
        config = PatchTSTConfig.from_candidate(candidate)

        self.assertEqual(config.lookback, 4)
        self.assertEqual(config.mask_fraction, 0.5)
        self.assertEqual(config.patch_length, 2)
        self.assertEqual(config.patch_stride, 1)
        self.assertEqual(config.transformer_depth, 1)
        self.assertEqual(config.attention_heads, 1)
        self.assertEqual(config.embedding_dim, 4)

    def test_config_rejects_patch_length_longer_than_lookback(self):
        with self.assertRaisesRegex(ValueError, "patch_length"):
            PatchTSTConfig(lookback=2, patch_length=3)
