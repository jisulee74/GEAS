...
from core.preprocessing import prepare_for_sensor_qc, normalize_for_derived, latest_row_for_controller
...
CONTROL_API_HISTORY_CHANNEL_TO_PRED = {
    1: "pred_ltw",
    2: "pred_rtw",
    4: "pred_pc1",
    5: "pred_pc2",
...
def _control_api_history_table_candidates() -> list[str]:
    configured = (
        os.getenv("CONTROL_API_HISTORY_TABLE", "")
        or os.getenv("ACTION_HISTORY_TABLE", "")
    ).strip()
    candidates = []
    if configured:
    ...
    candidates.extend(
        [
            "send_ds_control_api_history",
            "send_ds_control__api_history",
...
def fetch_today_control_api_history_dataframe(cfg: DbConfig) -> pd.DataFrame:
    now = kst_now()
    sod, _ = today_window_kst(now)
    limit = int(os.getenv("FETCH_LIMIT_TODAY", "5000"))
    history_db = (
        os.getenv("CONTROL_API_HISTORY_DB", "")
        or os.getenv("ACTION_HISTORY_DB", "")
    ).strip()
    time_candidates = ["reg_date", "send_date", "request_time", "created_at", "create_date", "timestamp"]
    farm_candidates = ["iot_data_idx", "farm_sn", "farm_id", "greenhouse_sn"]

    conn = db_connect_for_write(cfg)
    try:
        for table_name in _control_api_history_table_candidates():
            qualified_name = table_name
            if history_db and "." not in qualified_name:
                qualified_name = f"{history_db}.{qualified_name}"
            quoted_name = _quote_table_name(qualified_name)

            try:
                with conn.cursor(pymysql.cursors.DictCursor) as cur:
                    cur.execute(f"SHOW COLUMNS FROM {quoted_name}")
                    rows = cur.fetchall()
            ...
            if not time_col:
                continue
            farm_col = next((col for col in farm_candidates if col in columns), None)

            where = [f"`{time_col}` >= %s", f"`{time_col}` <= %s"]
            params = [sod.to_pydatetime(), now.to_pydatetime()]
            if farm_col:
                where.append(f"`{farm_col}` = %s")
                params.append(cfg.farm_sn)
...
def control_api_history_to_actuation_frame(df_history: pd.DataFrame) -> pd.DataFrame:
    if df_history.empty:
        return pd.DataFrame()
    ...
    time_col = _first(["reg_date", "send_date", "request_time", "created_at", "create_date", "timestamp"])
    channel_col = _first(["channel", "device_id"])
    ratio_col = _first(["ratio", "target", "value"])
    if not time_col or not channel_col or not ratio_col:
        return pd.DataFrame()

    work = df_history[[time_col, channel_col, ratio_col]].copy()
    work["reg_date"] = pd.to_datetime(work[time_col], errors="coerce")
    work["channel"] = pd.to_numeric(work[channel_col], errors="coerce").astype("Int64")
    work["ratio"] = pd.to_numeric(work[ratio_col], errors="coerce")
    work["actuation_col"] = work["channel"].map(CONTROL_API_HISTORY_CHANNEL_TO_PRED)
    work = work.dropna(subset=["reg_date", "actuation_col"])
    if work.empty:
        return pd.DataFrame()

    wide = (
        work.sort_values("reg_date")
        .pivot_table(index="reg_date", columns="actuation_col", values="ratio", aggfunc="last")
        .reset_index()
    )
    if "pred_ltw" in wide.columns or "pred_rtw" in wide.columns:
        wide["window_pct"] = wide[["pred_ltw", "pred_rtw"]].max(axis=1, skipna=True)
    return wide
...
def prepare_today_context(
    ...
    df_prepared = prepare_for_sensor_qc(
        df_today_raw,
        df_action_history=df_action_log,
        df_control_log=df_control_log,
    )
    df_calibrated = apply_sensor_calibration(df_prepared)
...
def run_once(
    *,
    ...
    try:
        df_action_log_today = fetch_today_control_api_history_dataframe(cfg)
    except Exception as e:
        print(f"[WARN] fetch control API history for action recovery failed: {e}")
        df_action_log_today = pd.DataFrame()

    try:
        df_control_log_today = fetch_today_named_table_dataframe(cfg, "control_actuation")
    except Exception as e:
        print(f"[WARN] fetch control_actuation for action fallback failed: {e}")
        df_control_log_today = pd.DataFrame()
    ...
    ctx = prepare_today_context(
        ...
        T_day=T_day,
        T_night=T_night,
        df_action_log=df_action_log_today,
        df_control_log=df_control_log_today,
    )
    if ctx is None:
    ...
    try:
        df_actuation_today = control_api_history_to_actuation_frame(df_action_log_today)
    except Exception as e:
        print(f"[WARN] build control API history frame for policy update failed: {e}")
...
def run_daily_batch(
    *,
    ...
    try:
        ...
        print(f"[WARN] fetch control API history for daily context failed: {e}")
        df_action_log_today = pd.DataFrame()

    try:
        df_control_log_today = fetch_today_named_table_dataframe(cfg, "control_actuation")
    except Exception as e:
        print(f"[WARN] fetch control_actuation for daily action fallback failed: {e}")
        df_control_log_today = pd.DataFrame()
