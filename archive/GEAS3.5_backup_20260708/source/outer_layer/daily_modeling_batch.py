...
def run_daily_modeling_batch(
    ...
    *,
    ...
    n_mc: int = 20,
    alpha: float = 0.9,
    enable_batch_qc: bool = True,
) -> Dict[str, Any]:
    batch_qc_report: Dict[str, Any] = {"status": "disabled"}
    df_modeling = df_history
    if enable_batch_qc:
        try:
            batch_qc = _load('batch_sensor_qc.py')
            df_modeling, batch_qc_report = batch_qc['run_batch_sensor_qc'](df_history)
        except Exception as exc:
            batch_qc_report = {
                "status": "failed",
                "error": str(exc),
                "fallback": "raw_history_used",
            }
            df_modeling = df_history

    classifier = _load('data_case_classifier.py')
    decision = classifier['classify_data_availability'](df_modeling, time_col='reg_date')
    valid_from, valid_to = _history_period(df_modeling)
    out: Dict[str, Any] = {
        'batch_qc': batch_qc_report,
        'case_info': {
