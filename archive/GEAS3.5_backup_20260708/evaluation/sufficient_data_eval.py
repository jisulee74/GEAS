"""
3.6.5. 異⑸텇???곗씠???섍꼍?먯꽌??寃利앸갑踰뺣줎

湲곗닠臾몄꽌 3.6.5??痍⑥???留욎떠, ?ㅼ젣 ?댁쁺 濡쒓렇瑜??ъ슜??
AI ?먯쑉?쒖뼱湲곗쓽 ?깅뒫/?덉쟾?깆쓣 ex-post 諛⑹떇?쇰줈 吏곸젒 ?됯??쒕떎.

?듭떖 ?뱀쭠
1. ?멸린/?닿린/?쒖뼱 濡쒓렇瑜??ㅼ륫媛믪쑝濡?洹몃?濡??ъ슜?쒕떎.
2. 諛섏궗???먮（???쒕??덉씠?섏씠??posterior ?섑뵆留곸쓣 ?섑뻾?섏? ?딅뒗??
3. KPI???ㅼ륫 ?대??섍꼍 沅ㅼ쟻?먯꽌 吏곸젒 怨꾩궛?쒕떎.
4. 蹂댁“ ?뚯깮吏??ACH, ?쒕갑?대웾, ?섍린?댁넀????異⑸텇?곗씠???먯텛??臾쇰━怨꾩닔濡?怨꾩궛?쒕떎.

寃곌낵 ?뚯씪 ?뺤떇? no_data_eval.py, limited_data_eval.py? 留욎텣??
"""
...
def regularize_time(df_std: pd.DataFrame, step_mins: int = 5) -> pd.DataFrame:
    ...
    # ?댁쭊 援щ룞 ?좏샇???됯퇏 ??> 0.5濡??먮Ⅴ硫?0/1 ?쇳빀 bin??0?쇰줈 ?щ씪吏????덈떎.
    # ex-post ?됯??먯꽌???대떦 吏묎퀎 bin 以???踰덉씠?쇰룄 援щ룞??寃쎌슦 ?쒖꽦?쇰줈 蹂몃떎.
...
def run_sufficient_data_eval(
    ...
    *,
    ...
    if min_steps_per_episode is None:
        # 5遺??먮뒗 ?꾨떖??step_mins) 吏묎퀎 湲곗??쇰줈, ???먰뵾?뚮뱶??80% ?댁긽??梨꾩썙議뚯쓣 ?뚮쭔 ?ъ슜?쒕떎.
