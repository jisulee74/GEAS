...
Step 9 starts after the AI Quality Model stage. Inputs default to
``03_quality_controlled`` and outputs are written to ``5_rl_dataset``. Rows with
unresolved action NaNs are excluded only here; this preserves preprocessing data
integrity while keeping the RL dataset action-safe.
"""
...
DEFAULT_DATASET_ROOT = (
    Path(__file__).resolve().parents[3] / "offline_dataset_preparation" / "datasets"
)
