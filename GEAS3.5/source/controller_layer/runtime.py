...
def prepare_today_context(
    ...
    df_prepared = prepare_for_sensor_qc(
        df_today_raw,
        df_action_log=df_action_log,
        df_weather_cache=df_weather_cache,
    )
...
def run_once(
    *,
    ...
    try:
        df_action_log_today = fetch_today_named_table_dataframe(cfg, "control_actuation")
    except Exception as e:
        print(f"[WARN] fetch control_actuation for action recovery failed: {e}")
        df_action_log_today = pd.DataFrame()

    weather_cache_table = os.getenv("WEATHER_CACHE_TABLE", "weather_api_cache")
    try:
        df_weather_cache_today = fetch_today_named_table_dataframe(cfg, weather_cache_table)
    except Exception as e:
        print(f"[WARN] fetch {weather_cache_table} for weather recovery failed: {e}")
        df_weather_cache_today = pd.DataFrame()
    ...
    ctx = prepare_today_context(
        ...
        df_weather_cache=df_weather_cache_today,
    )
    if ctx is None:
