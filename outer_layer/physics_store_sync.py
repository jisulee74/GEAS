...
def _resolve_updated_root() -> Path:
    default_root = Path(__file__).resolve().parents[1]
    return Path(os.getenv("GEAS_PROJECT_ROOT", str(default_root)))


def _load_store_api():
    updated_root = _resolve_updated_root()
    if str(updated_root) not in sys.path:
        sys.path.insert(0, str(updated_root))
    from controller_layer.physics_store import PhysicsStore, save_backend_params
...
def _store_path(path: Optional[str | Path] = None) -> Path:
    if path is not None:
        return Path(path)
    return _resolve_updated_root() / "controller_layer" / "physics_store" / "physics_params_store.json"
