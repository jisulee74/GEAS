"""
3.6.4. 제한적 데이터 환경에서의 부분 검증 방법론

기술문서의 의도에 맞춰 다음 절차를 수행한다.

1. 실측 외기/제어 로그를 전처리해 고정된 경계조건(boundary)과 관측 행동(action)을 만든다.
2. 정보량이 높은 이벤트 구간을 추출한다.
3. 이벤트 구간에서의 open-loop 재생 오차를 이용해 물리 파라미터 posterior를 근사한다.
4. posterior 샘플에 대해 같은 외기 경계조건에서 AI 정책을 counterfactual rollout 한다.
5. KPI 분포, q90, CVaR90를 저장한다.

주의:
- 문서에는 Particle MCMC posterior가 제시되지만, 본 구현은 현장 활용성을 위한
  "importance-resampling 기반 posterior 근사"를 사용한다.
- 핵심 구조는 문서와 동일하게 "실측 로그로 불확실성을 축소한 뒤, 고정된 외기
  경계조건에서 반사실 KPI의 꼬리위험을 평가"하는 흐름이다.
"""
...
_UPDATED_ROOT = Path(__file__).resolve().parents[1] / "GEAS3.0"
...
def project_action(a: Dict[str, float], prev_a: Dict[str, float], ramp_max: float = 0.15) -> Dict[str, float]:
    ...
    # 문서의 안전투영 취지에 맞춰 난방-환기 동시 사용은 금지한다.
...
def select_event_windows(
    ...
    *,
    ...
    # 문서의 제한적 데이터/부분 식별 설명에서 강조한 구간:
    # 야간 무일사, 히터 스텝, 환기 스텝, 맑은 주간
