...
try:
    ...
    eval_js('''
        ...
        window.addEventListener('error', (e) => {
            window._capturedErrors.push(e.message || e.error?.message || String(e));
        });
    ''')

    eval_js('window.switchExperiment("transition")')

    # Switch tabs back and forth 3 times!
    tabs = ['overview', 'ranking', 'onestep', 'safety', 'drift', 'resource', 'ablation']
    for cycle in range(3):
        for t in tabs:
            eval_js(f'window.switchTransitionTab("{t}")')

    errs = eval_js('window._capturedErrors')
    print("Errors after cycling tabs 3 times:", errs)
