...
try:
    ...
    for sub in ['ranking', 'safety']:
        eval_js(f'window.switchTransitionTab("{sub}")')
        res = eval_js(f'''
            (() => {{
                const html = document.getElementById("tab-panel-{sub}").innerHTML;
                const matches = [];
                for (const m of html.matchAll(/NaN/g)) {{
                    matches.push(html.slice(Math.max(0, m.index - 50), Math.min(html.length, m.index + 50)));
                }}
                return matches;
            }})()
        ''')
        print(f'=== {sub} NaN contexts ===')
        for snippet in res:
