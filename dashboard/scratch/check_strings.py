...
try:
    ...
    for sub in ['overview', 'ranking', 'onestep', 'safety', 'drift', 'resource', 'ablation']:
        eval_js(f'window.switchTransitionTab("{sub}")')
        res = eval_js(f'''
            (() => {{
                const html = document.getElementById("tab-panel-{sub}").innerHTML;
                return {{
                    hasNoMatching: html.includes("No matching records"),
                    hasNoAvailable: html.toLowerCase().includes("no available"),
                    hasJaryoEopseum: html.includes("자료 없음"),
                    hasNaN: html.includes("NaN"),
                    nanIndices: Array.from(html.matchAll(/NaN/g)).map(m => m.index),
                    noAvailableMatches: Array.from(html.matchAll(/no available/gi)).map(m => html.slice(Math.max(0, m.index - 30), Math.min(html.length, m.index + 50)))
                }};
            }})()
        ''')
        print(f'{sub}:', json.dumps(res, indent=2))
finally:
    proc.terminate()
