...
try:
    ...
    # 1. Desktop 1280px
    ws.call('Emulation.setDeviceMetricsOverride', {'width': 1280, 'height': 800, 'deviceScaleFactor': 1, 'mobile': False})
    time.sleep(0.3)
    d_cols = eval_js('getComputedStyle(document.querySelector("#container-exp-quality .metrics-grid")).gridTemplateColumns')
    print("Desktop 1280px grid columns:", d_cols, f"({len(d_cols.split())} columns)")

    # 2. Tablet 800px
    ws.call('Emulation.setDeviceMetricsOverride', {'width': 800, 'height': 800, 'deviceScaleFactor': 1, 'mobile': False})
    time.sleep(0.3)
    t_cols = eval_js('getComputedStyle(document.querySelector("#container-exp-quality .metrics-grid")).gridTemplateColumns')
    print("Tablet 800px grid columns:", t_cols, f"({len(t_cols.split())} columns)")

    # 3. Mobile 480px
    ws.call('Emulation.setDeviceMetricsOverride', {'width': 480, 'height': 800, 'deviceScaleFactor': 1, 'mobile': True})
    time.sleep(0.3)
    m_cols = eval_js('getComputedStyle(document.querySelector("#container-exp-quality .metrics-grid")).gridTemplateColumns')
    print("Mobile 480px grid columns:", m_cols, f"({len(m_cols.split())} column)")
