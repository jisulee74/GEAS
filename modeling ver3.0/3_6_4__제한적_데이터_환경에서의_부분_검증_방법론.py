...
실측 데이터를 활용한 open-loop 시뮬레이션 + Monte Carlo로
AI 제어 정책의 KPI를 부분 검증한다.
"""
...
def step_dynamics_dt(state: dict, u, a: dict, theta: dict,
    ...
    """실제 dt_sec를 사용하는 동역학 시뮬레이션 (실측 타임스텝 대응)."""
    Tin  = state["Tin"]
    e_in = state["e_in"]
    CO2  = state["CO2"]
...
def prepare_timeseries(data: pd.DataFrame, tz="Asia/Seoul") -> pd.DataFrame:
    """raw DB 데이터를 시뮬레이션에 필요한 형태로 전처리한다."""
    df = data.copy()
...
def make_inputs_actions(df: pd.DataFrame,
    ...
    """전처리된 DataFrame에서 외기 경계조건과 실측 행동을 분리한다."""
...
def simulate_openloop(boundary: pd.DataFrame, action: pd.DataFrame,
    ...
    """실측 행동 시퀀스를 그대로 재생(open-loop)하여 시뮬레이션한다."""
    n = len(boundary)
...
# ── 7. Monte Carlo 파라미터 샘플링 ──────────────────────────────────────────

def sample_theta(rng=None, *, auto_save: bool = False, farm_sn: int | None = None, stage_name: str | None = None, model_name: str = "default", store_path: str | None = None) -> dict:
    if rng is None:
...
def robust_openloop_mc(boundary: pd.DataFrame, action: pd.DataFrame,
                       ...
                       show_progress=True) -> pd.DataFrame:
    """
    N회 Monte Carlo 샘플링으로 open-loop KPI 분포를 계산한다.
    """
